...
export default function ConfirmDeleteBookingModal({ bookingId, customerName, isOpen, onClose }: ConfirmDeleteBookingModalProps) {
    ...
    const handleDelete = async () => {
        try {
            ...
            if (res.success) {
                toast.success(`Successfully deleted booking for "${customerName}"`);
                onClose();
            }
        } catch (error: any) {
            toast.error(error?.data?.message || `Failed to delete booking for "${customerName}"`);
        }
    };
            ...
            <div className="relative bg-white w-full max-w-md overflow-hidden rounded-[2.5rem] shadow-2xl flex flex-col animate-in zoom-in-95 duration-200">
                ...
                <div className="p-10 space-y-8 text-center">
                    ...
                    <div className="space-y-3">
                        <h2 className="text-3xl font-black text-slate-900 tracking-tight">Remove Booking</h2>
                        <p className="text-slate-500 font-medium leading-relaxed">
                            Are you absolutely sure you want to remove the booking for <br /> 
                            <span className="text-red-500 font-black italic">"{customerName}"</span>?
                        </p>
                        <div className="bg-amber-50 border border-amber-100 p-4 rounded-2xl flex gap-3 text-left animate-in slide-in-from-top-2 duration-500">
                            ...
                            <p className="text-[11px] text-amber-700 font-bold leading-relaxed">
                                ...
                                The booking record will be hidden from all dashboard views.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-3 pt-2">
                        ...
                        >
                            ...
                            {isDeleting ? "Deleting Booking..." : "Confirm Deletion"}
                        </button>
